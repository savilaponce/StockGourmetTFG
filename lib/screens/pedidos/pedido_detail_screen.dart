...
class _PedidoDetailScreenState extends ConsumerState<PedidoDetailScreen> {
  late Future<Pedido> _pedidoFuture;
  ...
  void _loadPedido() {
    _pedidoFuture = ref.read(pedidoServiceProvider).getById(widget.pedidoId);
  }

  void _reload() {
    setState(() => _loadPedido());
  }

  @override
  Widget build(BuildContext context) {
    final lineasAsync = ref.watch(pedidoLineasProvider(widget.pedidoId));
      ...
      builder: (context, snapshot) {
        ...
        return Scaffold(
          ...
          body: snapshot.connectionState == ConnectionState.waiting
              ? const Center(child: CircularProgressIndicator(color: SGColors.primary))
              : snapshot.hasError
  ...
  Future<void> _recibir(BuildContext context) async {
    ...
    if (confirmed == true) {
      try {
        await ref.read(pedidoServiceProvider).recibirPedido(widget.pedidoId);
        ref.invalidate(pedidosProvider);
        ref.invalidate(pedidoLineasProvider(widget.pedidoId));
        if (mounted) {
  ...
  Future<void> _cancelar(BuildContext context) async {
    await ref.read(pedidoServiceProvider).cambiarEstado(widget.pedidoId, 'cancelado');
    ref.invalidate(pedidosProvider);
  ...
  Future<void> _eliminar(BuildContext context) async {
    ...
    if (confirmed == true) {
      await ref.read(pedidoServiceProvider).delete(widget.pedidoId);
      ref.invalidate(pedidosProvider);
      if (mounted) context.pop();
    }
  }
